...
/// For Smart
ll dpMAX(int i , int j)
{
    if(i>j)
        return 0;
    if( DP[i][j]!= -1 )
        return DP[i][j] ;
    sum = max( arr[i] + min(dpMAX(i+1,j-1),dpMAX(i+2,j)) , arr[j] + min(dpMAX(i,j-2),dpMAX(i+1,j-1)));
DP[i][j] = sum ;
    return sum ;
}

/// For Dump
ll dpMIN(int i , int j)
{
    if(i>j)
        return 0;
    if( DP[i][j]!= -1 )
        return DP[i][j] ;
    sum = max( arr[i] + max(dpMIN(i+1,j-1),dpMIN(i+2,j)) , arr[j] + max(dpMIN(i,j-2),dpMIN(i+1,j-1)));
DP[i][j] = sum ;
    return sum ;
}
