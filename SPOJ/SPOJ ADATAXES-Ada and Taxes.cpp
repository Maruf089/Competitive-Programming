...
ll n,m,b,q,t,i,l,r,j,d,cs=0,counT=0,k,sum1=0,sum=0,Max,Min,num;
...
int main()
{
    ...
    f0(i,n)
    {
        update(1,0,n-1,i,1); /// build segment tree with every node 1
    }

    f0(i,q)
    {
        while(vc.sz>0 and vc[0].F <= que[i].first)
        {
            update(1,0,n-1,vc[0].S,vc[0].first);
            vc.pop_front();
        }
        ans[que[i].second] = query(1,0,n-1,L[que[i].S],R[que[i].S]);
    }
    f0(i,q)
        cout << ans[i] << endl;
