...
vector<ll>vc[mx];
ll in[mx],out[mx],val[mx];

void dfs(ll node,ll parent) /// compute in[] array
{
    in[node] = 0 ;
    for(auto next : vc[node])
    {
        ...
        dfs(next,node);
        counT = 0 ;
        if( val[next] - val[node] > 0 )
            counT = val[next]-val[node] ;
        in[node] = max(in[node],in[next]+counT);
    }
}
void dfs2(int node, int parent) /// compute out array
{
    ...
    for(auto next : vc[node])
    {
        ...
        if(in[next]+counT>=Max1)
            Max2 = Max1,Max1 = in[next]+counT;
        else if(in[next]+counT>Max2)
            Max2 = in[next]+counT;
    ...
    for(auto next : vc[node])
    {
        ...
        if(Max1==in[next]+counT)
            use = Max2;

        counT = 0 ;
        if( val[node] - val[next] > 0 )
            counT = val[node] - val[next] ;

        out[next] = max(counT+out[node],counT+use); /// 1+max(out[p],use+1)
//dbg(node,next,Max1,Max2,use,out[next]);
        dfs2(next,node);
    }
}
int main()
