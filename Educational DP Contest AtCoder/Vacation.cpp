...
ll DP()
{
    ...
    for(i=1; i<n; i++)
    {
          dp[i][0] = max(dp[i-1][1]+vc[i][0],dp[i-1][2]+vc[i][0]);
          dp[i][1] = max(dp[i-1][0]+vc[i][1],dp[i-1][2]+vc[i][1]);
          dp[i][2] = max(dp[i-1][0]+vc[i][2],dp[i-1][1]+vc[i][2]);
