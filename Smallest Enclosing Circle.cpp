...
/// Enclosing Ball O(N*Constant Number) or Enclong Circle( just delete z part )
...
int32_t main()
{
    ...
    while (it--)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            if (dist(X, Y, Z, x[i], y[i], z[i]) > dist(X, Y, Z, x[id], y[id], z[id]))
                id = i;
        }
        X = X + (x[id] - X) * ratio;
        Y = Y + (y[id] - Y) * ratio;
    ...
    for (int i = 0; i < n; i++)
    {
        long_dis = max(long_dis, sqrt(dist(x[i],y[i],z[i],X,Y,Z)));
    }
    cout << fixed << setprecision(10) << long_dis << '\n';
}
