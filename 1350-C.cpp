...
int main()
{
  ...
  fastio;
      ...
      cin >> n ;
      ll arr[n+9] , preffix_gcd[n+9] , temp_gcd[n+9];
      f1(i,n)
      {
          cin >> arr[i];
      }
      ...
      for(i=n-1;i>=1;i--)
      {
          preffix_gcd[i] = __gcd(preffix_gcd[i+1],arr[i]);
      }
      f1(i,n-1)
      {
         temp_gcd[i] = (arr[i]*preffix_gcd[i+1]) / __gcd(arr[i],preffix_gcd[i+1]);
      }
      ans = temp_gcd[1];
      for(i=2;i<n;i++)
        ans = __gcd(ans,temp_gcd[i]);
      cout << ans << endl;
