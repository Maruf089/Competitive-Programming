...
int x, n,m,a,b,t,i,j,d,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;

int brute_force(int pos,vector<int>&factors,vector<int>arr,int k)
{
    if(pos==factors.sz)
        {
            int sum = 0;
    ...
    {
        arr[i] *= factors[pos];
        ans = min(ans,brute_force(pos+1,factors,arr,k));
        arr[i] /= factors[pos];
    }
...
int main()
{
    ...
    while(t--)
    {
      ...
      //  dbg(factors);
        ...
        if(factors.sz<=k) /// sob add + baki gula 1
        {
            int sum = 0;
            for(auto it : factors)
                sum += it;
            counT = sum + k - factors.sz;

        }
        else /// brute force kore minimum ta ans
        {
            vector<int>vc(k,1);
            counT = brute_force(0,factors,vc,k);
        }
