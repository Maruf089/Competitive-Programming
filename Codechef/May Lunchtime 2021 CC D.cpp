...
void printOrderedSet(new_data_set s){
...
void myerase(new_data_set &t, int v){
    int rank = t.order_of_key(v);//Number of elements that are less or equal to v in t
    new_data_set::iterator it = t.find_by_order(rank); //Iterator that points to the (rank)th element in t
    t.erase(it);
}
...
/// randomely select pos in a range(l,r)
/*
    ...
    while(t--)
    {
        ...
        for(i=n;i>=1;i--)
        {
            for(j=0;j<=k;j++)
            {
                ll op1 = dp[i+1][j][0];
                ll op2 = dp[i+1][j+1][1];
                dp[i][j][0] = max( op1 , op2 + arr[i] *(j+1) );

                ll op3 = dp[i][j][0];
                ll op4 = dp[i+1][j][1];
                dp[i][j][1] = max( op3 , op4 + arr[i] * j );
            }
        }
...
ll go(int i,int j,bool ok)
{
    ...
    ll ans = arr[i] * (j+1) + go(i+1, j, 1 ) ;   /// same partition
    ans = max( ans, arr[i]*(j+1) + go(i+1, j+1, 0) );    /// new partition start

    if(ok)
        ans = max( ans, go(i,j+1,0));
    else
        ans = max(ans, go(i+1,j,0) );
