...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i=0; i<n; i++)
        {
            ...
            cin >> obj[i].id >> obj[i].sol >> obj[i].pen >> obj[i].M;
            sum =  0 ;
            for(j=0; j<obj[i].M; j++)
            {
                cin >> a;
    ...
    int prev = 0 , OK = 0 ;
        for(i=1; i<Rank.size(); i++)
        {
            ...
            for(j=0; j<n; j++)
            {
                if(Rank[i]==obj[j].id)
                {
                   if( obj[j].sol <= obj[prev].sol && obj[j].pen <= obj[prev].pen )
                   {
                       OK = 1 ;
                       while(obj[j].sol <= obj[prev].sol && obj[j].pen <= obj[prev].pen && obj[j].M>=1)
                       {
                           obj[j].sol++;
                           obj[j].pen  += vc[j][index];
                           index++;
                       }
                   }
