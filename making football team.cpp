     ...
     // cin >> t ;
      while(t--)
      {
          ...
          while(m--)
          {
              ll x, k ;
              cin >> x >> k ;
              int pos = lower_bound(all(vc),x) - vc.begin();
              int res = vc.sz - pos ;

     //   dbg(pos,res,x,k);
              int low = 1 , high = pos , option = pos + 1 ;

              while(low<=high)
              {
                  int mid = (low+high)>>1;
                  ll sum_ase = precal[pos] - precal[mid-1];
                  ll dorkar = 1LL*x*(pos-mid+1);
                  ll baki = (dorkar-sum_ase);
  //  dbg(mid,sum_ase,dorkar,baki);
                  if(baki<=k)
                  {
                      high = mid-1;
                      option = mid;
