...
int main()
{
    ll n,m,a,b,i,j,counT=0,k,ones=0,zeros=0,temp=0,sum=0,Max,Min,num;
    cin>>s>>s2;

    if(s2[0]=='0' && s2.size()!=1) return cout << "WRONG_ANSWER", 0;
...
/// cout<< ones<< " " << zeros << endl;
    ...
    lp(i,zeros,s.size())
    {
        if(i==zeros and s[i]=='1')
            continue;
        else
            s1+=s[i];
    }

    s1==s2 ? cout << "OK" : cout << "WRONG_ANSWER" << endl;
