...
int main()
{
  ...
  f0(i,H)
  {
      f0(j,W)
      {
          if(grid[i][j])
          {
              vector<vector<int>>dis(21,vector<int>(21,inf));
              queue<pii>Q;
              Q.push({i,j});
              dis[i][j] = 0 ;
              while(!Q.empty())
              {
                  ...
                  if(w+1<W and grid[h][w+1] and dis[h][w+1]==inf)
                  {
                      dis[h][w+1] = dis[h][w] + 1 ;
                      Q.push({h,w+1});
                      Max = max(Max,dis[h][w+1]);
                  }
                  if(h+1<H and grid[h+1][w] and dis[h+1][w]==inf)
                  {
                      dis[h+1][w] = dis[h][w] + 1 ;
                      Q.push({h+1,w});
                      Max = max(Max,dis[h+1][w]);
                  }
                  if(w>0 and grid[h][w-1] and dis[h][w-1]==inf)
                  {
                      dis[h][w-1] = dis[h][w] + 1 ;
                      Q.push({h,w-1});
                      Max = max(Max,dis[h][w-1]);
                  }
                  if(h>0 and grid[h-1][w] and dis[h-1][w]==inf)
                  {
                      dis[h-1][w] = dis[h][w] + 1 ;
                      Q.push({h-1,w});
                      Max = max(Max,dis[h-1][w]);
