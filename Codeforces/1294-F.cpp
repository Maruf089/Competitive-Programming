...
// order_of_key(x) – ফাংশনটি x এলিমਠƨ্টটা কোন পজিশনে আছে সেটা বলে দেয়।
...
/// ************************************** Code starts here ****************************************** */
string s,s1;
ll a,b,n,m,i,j,d,t,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;
...
int main()
{
  //  freopen("in.txt","r",stdin);
    ...
    {
        cout << n-1 << endl;
        cout << diameter[0]+1 << " " << diameter[1]+1 << " " << diameter.back()+1 << ln ;
    }
    else
    {
        queue<int>Q;
        vector<int>dd(n,-1);
        for(auto node : diameter)
        {
            dd[node] = 0 ;
            Q.push(node);
        }
...
// dbg(diameter.size(),mx.F);
        cout << diameter.sz-1+mx.F << ln ;
        cout << diameter[0]+1 << " " << diameter.back()+1 << " " << mx.S+1 << ln ;
    }
}
