...
/// ************************************** Code starts here ****************************************** */
string s,s1;
ll n,m,a,b,i,j,d,len,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;
...
int main()
{
    ...
    TC
    {
        ...
        f0(i,n)
        {
            ...
            if(s[i]=='L')
                tie(d,k) = make_pair(d+fx[0],k+fy[0]);
            else if(s[i]=='R')
                tie(d,k) = make_pair(d+fx[1],k+fy[1]);
            else if(s[i]=='U')
                tie(d,k) = make_pair(d+fx[2],k+fy[2]);
            else if(s[i]=='D')
                tie(d,k) = make_pair(d+fx[3],k+fy[3]);

              if(mp1.find({d,k})!= mp1.end())
                {
                    int lf = mp1[{d,k}];
                    int rr = i+1;
                    len = rr-lf;
