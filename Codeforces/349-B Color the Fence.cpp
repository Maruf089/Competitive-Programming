...
/// ************************************** Code starts here ****************************************** */
string s,s1;
ll n,m,a,b,i,j,d,t,cs=0,digit,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;
map<ll,ll>mp;
...
int main()
{
  //  freopen("in.txt", "r", stdin);
     ...
     for(i=9;i>=1;i--)
     {
         ...
         ll check = mp[i]-mp[res[idx]];
 //   dbg(mp[i],res[idx],check,rem);

         if(check<=rem and rem>0 and check>0)
         {
             rem -= check;
             res[idx] = i ;
        idx++;
