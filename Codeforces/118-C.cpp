...
ll n,m,a,b,t,i,j,d,cs=0,counT=0,k,sum1=0,sum=0,Max,num;
vector<ll>vc;
map<ll,ll>mp;
...
int main()
{
    ...
    for(i=0; i<10; i++)
    {
        ...
        for(j=0; j<10; j++)
        {
            ...
            {
                if( rem>0 and s[l]-'0' == (i+j))
                    res+=j,temp[l] = i + '0', rem--;
            }
   // dbg(i,j,temp,res,rem,"----\n");
            if(j==0)
                continue;
            for(int l=s.size()-1;l>=0;l--)
            {
                if( rem and s[l]-'0'==(i-j))
                    res+=j,temp[l] = i+'0',rem--;
            }
 //   dbg(i,j,temp,res,rem,"----\n");
        }
//dbg(res,temp);
        if( res < Min || (res==Min and temp <ans))
            Min = res , ans = temp ;
    }
    cout << Min << '\n' << ans << ln ;
