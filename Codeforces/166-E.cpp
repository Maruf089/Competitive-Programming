...
const int mx = (int)1e7+9;

ll n,m,a,b,t,i,j,d,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;


ll dp[2][mx];
ll solve(ll same,ll steps)  /// Recursive will give MLE
{
    if(steps==1)
     {
         if(same)
    ...
    ll &ans = dp[same][steps];
    if(ans!=-1)
        return ans;
   if(same==1)
    return ans = (3LL*solve(0,steps-1))%MOD;
    else
    return ans = ((2LL*solve(0,steps-1))%MOD + (solve(1,steps-1))%MOD)%MOD;
}
