...
ll n1,n2,m,k1,k2,a,b,t,i,j,d,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;

ll dp[100+5][100+5][10+6][10+6];

ll func(int f,int h,int kf,int kh)
{
    ll &ret = dp[f][h][kf][kh];
    if(ret!=-1)
        return ret;
    else
    {
        ...
        else
        {
            ll ans = 0 ;
            if( f>0 and kf>0 )
                ans = ( ans + func( f-1,h,kf-1,k2 ) ) % MOD ;
            if( h>0 and kh>0 )
                ans = ( ans + func( f,h-1,k1,kh-1 ) ) % MOD ;
            ret = ans;
        }
        return ret;
    }
