...
/// ************************************** Code starts here ****************************************** */
string s,s1;
ll n,m,b,i,j,d,t,cs=0,counT=0,k,ans=-1,l=0,sum1=0,sum=0,Max,Min,num;
vector<ll>vc;
map<ll,ll>mp;
deque<pll>q;
int main()
{
    ...
    f0(l,n)
    {
        ...
        if(!q.empty())
            curmax = q.front().first;

        while(r<nn and a[r]*2>=curmax)
        {
            while(!q.empty() and q.back().first<a[r])
                q.pop_back();
            q.push_back({a[r],r});
            curmax = q.front().first;
            ++r;
