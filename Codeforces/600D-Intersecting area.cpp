...
long double segment(long double r,long double theta)
{
...
int main()
{
    ...
    cin >> x >> y >> r >> x1 >> y1 >> r1;
    long double d = distance(Point(x, y), Point(x1, y1));
    if (d + r <= r1)
    {
    ...
    if (d + r1 <= r)
    {
        cout << fixed << setprecision(20) << pi * r1 * r1 << endl;
        return 0;
    }
    if (d >= r + r1)
    {
    ...
    long double theta = 2 * acos((r * r + d * d - r1 * r1) / (2.0 * r * d)) ;
    long double ans = segment(r, theta);
    long double theta1 = 2 * acos((r1 * r1 + d * d - r * r) / (2.0 * r1 * d)) ;
    ans += segment(r1, theta1);
