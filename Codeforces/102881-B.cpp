...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i=0; i<3; i++)
        {
            a = arr[i], b = arr[(i+1)%3],  c = arr[(i+2)%3];

            double l = max(0.0, abs(a-b)-c), r = min( max(0.00,a+b-c), k*1.00 );

            double res = 0.00 , iteration = 0;

            while (l < r + EPS and iteration<400)
            {
                iteration++;
                double m1 = (2*l+r) / 3.00;
                double m2 = (l+2*r) / 3.00;
                double f1 = findArea(a, b, c+m1);
                double f2 = findArea(a, b, c+m2);
