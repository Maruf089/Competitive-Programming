...
ll n,m,a,b,t,i,j,d,cs=0,r,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;
vector<ll>vc;
map<ll,ll>mp;

int include(int i)
{
    return (i>=l and i<=r) ? 1 : 0 ;
}
int dp[2050][2050];
int solve(int i,int j)
{
    ...
    int kom = (j+vc[i]-1+m)%m;
    int nakom = (j+vc[i]+m)%m;
    dp[i][j] = include(kom) + solve(i+1,kom);
    dp[i][j] = max(dp[i][j],include(nakom)+solve(i+1,nakom));
