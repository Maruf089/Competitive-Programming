...
const int mx = (int)20000001+9;

ll n,m,c,x,a,b,t,i,j,d,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;

vector<ll>distinct_prime(mx+9);
void solve()
{
    ll c,d,x;
    cin>>c>>d>>x;
    ...
    auto get = [&](ll y)
    {
        ...
        if(z % (c*y))
            return 0ll;
        z /= c*y;
        return (1LL<<distinct_prime[z]);
    };
    for(ll gcd = 1; gcd*gcd<=x; ++gcd)
    {
        if(x%gcd == 0)
        {
            ans += get(gcd);
