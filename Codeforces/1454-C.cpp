...
main()
{
    ...
    {
        inp(n); ll arr[n+9];
        map<ll,ll>mp,index,res,chk;
        int idx =  0 , Min = INF , ans = INF;
        f1(i,n)
        {
            inp(a);arr[i] = a;mp[a]++;
            if(mp[a]==1)
            ...
            {
                int prev = index[a]; int diff = i - prev ;
                if(diff>1) res[a]++;
                index[a] = i;
            }
        }
        f1(i,n)
        {
            ...
            if(mp[arr[i]]==1 and chk[arr[i]]==0)
            {
                chk[arr[i]]++;
                if(index[arr[i]]==1 or index[arr[i]]==n  ) ans = 1;
                else ans = min(ans,2LL);
            }
            else if(chk[arr[i]]==0)
            {
                chk[arr[i]]++;
                if(index[arr[i]]!=n) res[arr[i]]++;
