...
int main()
{
    ...
    while(t--)
    {
        ...
        ll cur_pos = 0 , cur_time = 0 , dest = 0,ans = 0;
        f0(i,n)
        {
            ll dx = dest - cur_pos;
            ll dt = T[i] - cur_time;
            ll direction = dx > 0 ? 1 : -1;

            ll steps = min(abs(dx),dt);
            cur_time += dt;
            cur_pos += steps*direction;

            if(cur_pos==dest) /// robot not moving , command received
                dest = X[i];
            /// else moving and ignore command

            if( min(cur_pos,dest) <= X[i] and X[i]<= max(cur_pos,dest) )
            {
                if( abs(cur_pos - X[i]) <= T[i+1] - T[i] )
                    ans++;
            }
