...
ll dfs(int steps,int i,int j)
{
    ...
    ret = INF;
    if(i-1>=0)
        ret = min( ret, dfs(steps-1, i-1, j ) + val[i][j][U]);
    if(j-1>=0)
        ret = min( ret, dfs(steps-1, i, j-1 ) + val[i][j][L]);
    if(i+1 < n)
        ret = min( ret, dfs(steps-1, i+1, j ) + val[i][j][D]);
    if(j+1 < m)
        ret = min( ret, dfs(steps-1, i, j+1 ) + val[i][j][R]);
