...
ll fact[mx+9], inv[mx+9] ;
ll fact_inv() /// O(n)
{
    fact[0] = inv[0] = 1;
    for(i=1; i<mx; i++)
        fact[i] = modMul(fact[i-1],i);
    inv[mx-1] = modPow(fact[mx-1],MOD-2);
    for(i=mx-2; i>=1; i--)
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(auto it : vc)
        {
            ...
            {
                ll AND = rangeAnd(0,id-1);
                AND = AND & rangeAnd(id+1,n-1);
                if(AND!=it.F)
                    continue;
            }

            k = modDiv( fact[mp[it.F]], modMul(fact[2], fact[mp[it.F]-2]) ) ;
          //    dbg(it,k,counT);

            d = n-2;
            d = modMul(fact[d],k*2);
            counT = modAdd(counT,d);
        }
