...
/// ************************************** Code starts here ****************************************** */
string s,s1;
int n,m,a,b,i,j,d,t,c=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;
vector<ll>vc;
map<ll,ll>mp;
...
int main()
{
    ...
    while(t--)
    {
        cin >> a >> b >> c ;
        Max =max(a,max(b,c));
        int low = 1, high = a+b+c;
        ans = 1 ;
        while(low<=high)
        {
            int mid = (low+high)/2;
            int x=m,y=m,z=m;
            int abaki = a - min(a,mid);
            x = max(mid - a,0) ;
            int bbaki = b - min(x,b) ;
//D(abaki);D(bbaki);D(x);
            int cbaki = c- min(c,mid);
            z = max(mid-c,0);
            bbaki = bbaki - min(z,bbaki);

            bool flag = true;
            if(abaki>0 and cbaki>0)
                flag = false;
            if((abaki+bbaki>mid) || (bbaki+cbaki>mid))
                flag = false;
