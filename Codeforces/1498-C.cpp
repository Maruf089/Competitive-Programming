...
ll dp[1003][1003][2];
ll solve(int pos,int age,int side)
{
    ll &ret = dp[pos][age][side];
    if(ret != -1 )
        return ret;
    ...
    if(side==0)
    {
        ans1 = 1LL + (pos-1 >= 1 ? solve(pos-1,age-1,1) : 0LL);
        ans2 = (pos+1 <= n ? solve(pos+1,age,0) : 0LL) ;
    }
    else
    {
        ans1 = (pos-1 >= 1 ? solve(pos-1,age,1) : 0LL);
        ans2 = 1LL + (pos+1 <= n ? solve(pos+1,age-1,0) : 0LL) ;
    }

    ret = modAdd(ans1,ans2);
    return ret;
}
int main()
{
