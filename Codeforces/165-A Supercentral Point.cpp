...
int main()
{
    ll n,m,a,b,i,j,counT=0,k,l=0,r=0,up=0,down=0,temp=0,sum=0,Max,Min,num;
    cin>>n;
    ll x[n],y[n];
    ...
    for(i=0; i<n; i++)
    {
        ...
        for(j=0; j<n; j++)
        {
            ...
            if( x[j]==x[i] and y[j]<y[i] )down++;
            if( x[j]==x[i] and y[j]>y[i] )up++;


            if(r and l and up and down)
            {
                if( (r+l+up+down) >= 4 )
                {
                    l=0,r=0,up=0,down=0;
                 ///   cout<<"it is "<<x[i]<<" "<<y[j]<<endl;
                    counT++;
                    break;
                }
            }
