...
int freq[Maxv], fav[Maxv], h[Maxm];
int dp[505][Maxv];

int Solve(int player, int card)
{
    if(player < 1)
        return 0;
    if(dp[player][card]!=-1)
        return dp[player][card];
    int mx = 0;
    for(int i = 0; i <= min(k,card); i++)
        mx = max(mx, h[i]+Solve(player-1, card - i));
    return dp[player][card] = mx;
}
