...
ll n,m,a,b,t,i,j,d,cs=0,counT=0,k,ans=0,l=0,sum1=0,num;
...
int main()
{
    ...
    while(t--)
    {
        ...
        mp[0] = mp[1] = mp[2] = mp[5] = mp[8] = 1 ;
        mpp[0]++;
        mpp[1]++;
        mpp[2] = 5, mpp[5] = 2,  mpp[8] = 8;
        inps(str);

        k = ( (str[0]-'0')*10 + (str[1]-'0') ) * m +  ( (str[3]-'0')*10 + (str[4]-'0') );
        vector<ll>vc;
        vc.pb(0);
        ...
        for(i = 1; i < (n*m); i++)
        {
            ...
            if(mp[d1] and mp[d2] and mp[d3] and mp[d4])
            {
                if(d4>1) d4 = mpp[d4];
                if(d3>1) d3 = mpp[d3];
                if(d2>1) d2 = mpp[d2];
        ...
        auto it = lower_bound(all(vc),k);
        if(it==vc.end()) it = vc.begin();
