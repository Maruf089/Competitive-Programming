...
char str[mx];
ll hash_val[2][mx], pw[2][mx], mod[2] = {1000000005,1000000007};
void create_hash_val()
{
    ...
    for(int id=0; id<2; id++)
    {
        for(i=1; i<=n; i++)
        {
            hash_val[id][i] = ((hash_val[id][i-1]*10)%mod[id] + str[i-1]-'0')%mod[id];
        }
    }
}
bool solve(ll a,ll b,ll c)
{
    ...
    if(max(a,b)>c)
        return 0;
    if(str[a]=='0' and b>1 or str[a+b]=='0' and c>1)
        return 0;
    ll id,A,B,C;
    f0(id,2)
    {
