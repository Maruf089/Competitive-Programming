...
ll n,m,a,b,t,i,j,d,cs=0,counT=0,k,ans=0,l=0,Min,num;
string s;
...
bool KMP(string text,string pattern,int idx)
{
    bool found = false;
...
int main()
{
   ...
   // dbg(s);
    ...
    if(Final[0]=='#' or Final[Final.sz-1]=='#')
        Max--;
    if(Final[0]=='#')
        word--;
    if(Final[Final.sz-1]=='#')
        word--;
    //dbg(s.sz , Max , word , Final);

    cout << s.sz - 2*Max + 2*word << ln;
}
