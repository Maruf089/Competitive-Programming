...
int main()
{
    ...
    while(t--)
    {
        ...
        f0(i,n)
        {
            if(i==0)
            {
                vc.pb(arr[i]);vc1.pb(arr[i]);
                tm = max(tm,arr[i]);Max.erase(arr[i]);Min.erase(arr[i]);
            }
            else
            {
                if(tm==arr[i])
                {
                    vc.pb(*Min.begin());Min.erase(Min.begin());
                    auto it = Max.lower_bound(arr[i]);it--;
                    vc1.pb(*it);Max.erase(it);
                }
                else
                {
                    vc.pb(arr[i]);vc1.pb(arr[i]);
