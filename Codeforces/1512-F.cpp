...
ll n,m,t,i,j,d,c,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;
...
int main()
{
    ...
    {
        cin >> n >> c;
        vector<int> a(n) , jaite_koto_din_lagbe(n);
        vector<int> b(n - 1);
        for (int &e : a)
            cin >> e;
        for (int &e : b)
            cin >> e;
        b.pb(0);
        ll ans = INF , cur = 0 , bal = 0;
        f0(i,n)
        {
            ll vagfol =  max(0LL , c - bal + a[i] - 1);
         //   dbg(bal , vagfol);

            ans = min(ans , cur + vagfol / a[i]);
       //     dbg(cur , cur + vagfol / a[i]);

            ll newdays = max(0LL , b[i] - bal + a[i] - 1) / a[i];
      //      dbg(newdays);

            cur = cur + newdays + 1 ;
            bal = bal + a[i]*newdays - b[i];
