...
int main()
{
    ...
    f1(i,10000)
    {
        ll mid = i ; ll chk = (mid*(mid+1))/2;
        mp[chk] = i;mp[chk-1] = i+1;
        vc.pb(chk);
    }
    ll lower=-1,up=-1 , low_val=-1,up_val=-1;
    cin >> t ;
    while(t--)
    {
       ...
       auto low = upper_bound(all(vc),n);
       up = low - vc.begin(); low--;
       lower = low - vc.begin();
        cout << min(lower+1,up+1) << ln;
