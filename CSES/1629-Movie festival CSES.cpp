...
int latestNonConflict(int i)
{
    ...
    while(low<=high){
    int mid = (low+high)/2;
    if(ob[mid].finish<=ob[i].start){
        if(ob[mid+1].finish<=ob[i].start)
        low = mid+1;
        else return mid;
    }
    else high = mid-1;
...
int main()
{
   ...
   f0(i,n)
   {
      cin >>  ob[i].start >> ob[i].finish;
   }
   sort(ob,ob+n,cmp);
//   f0(i,n)
//   {
//      cout <<  ob[i].start << " " << ob[i].finish << ln;
//   }
   table[0] = 1 ; /// 1st one is included
   for(int i=1;i<n;i++)
   {
       int include_movie = 1 ;
       int temp = latestNonConflict(i);
       if(temp!=-1)
        include_movie+=table[temp];
       table[i] = max(include_movie,table[i-1]);
   }
