...
int main()
{
    ...
    while(i<j)
    {
        ll L1 = abs(l-vc[i].F), R1 = abs(r-vc[i].F);
        ll L2 = abs(l-vc[j].F), R2 = abs(r-vc[j].F);
  //  dbg(vc[i].F,vc[j].F,l,r);
        ...
        if(L1>R1 or L2>R2)
        {
            ...
            {
                sum += L1 ;
                DQ.push_front(vc[i].S);
                l = vc[i++].F;
            }
            else
            {
                sum += L2;
                DQ.push_front(vc[j].S);
                l = vc[j--].F;
            }
        }
        else
...
//        cout << ln ;
    ...
    if(abs(l-vc[i].first)>abs(r-vc[i].first))
        DQ.push_front(vc[i].S),sum += abs(l-vc[i].F);
    else
        DQ.push_back(vc[i].S),sum += abs(r-vc[i].F);
//dbg(sum);
    cout<<sum<<"\n";
    while(!DQ.empty())
    {
