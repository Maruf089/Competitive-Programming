...
bool floatEqual(double _x1, double _x2)
{
    return fabs(_x1-_x2)<EPS;
}
//Returns true if _x1>_x2
bool floatGreater(double _x1, double _x2)
{
    return _x1-EPS>_x2;
}
//Returns true if _x1<_x2
bool floatLesser(double _x1, double _x2)
{
    return _x1+EPS<_x2;
}
...
bool doIntersect(point p1, point p2, point p3, point p4, point &p)
{
    ...
    {
        if(onSegment(p1, p3, p4))
            p.x=p1.x, p.y=p1.y;
        else if(onSegment(p2, p3, p4))
            p.x=p2.x, p.y=p2.y;
        else
            return 0;
        return 1;
    ...
    if(fabs(l1.b) > EPS)
        p.y = -(l1.a * p.x + l1.c);  // to avoid divide by zero
    else
        p.y = -(l2.a * p.x + l2.c);
    return 1;
}
...
double distToLineOrSeg(point p, point a, point b, point &c, bool _segment)
{
    ...
    if(_segment)
    {
        if(u<0.0)
            ...
            c=translate(a, scale(ab, u)); // Similar to line
    }
    else
...
{
    return fabs(crossProduct(toVector(p, q), toVector(p, r))) < EPS;
}
...
vector< point > ConvexHull(int n, point ara[])
{
    ...
    for(i=0, k=0; i<n; ++i)
    {
        while(k>=2 && cross(cnvx[k-2], cnvx[k-1], ara[i])<=0)
            k--;
        cnvx[k++]=ara[i];
    }
//    for(i=n-2, j=k+1; i>=0; --i)
//    {
//        while(k>=j && cross(cnvx[k-2], cnvx[k-1], ara[i])<=0)
//            k--;
//        cnvx[k++]=ara[i];
//    }
    cnvx.resize(k);   // Not taking the last point as first point
    return cnvx;
}
...
int main()
{
    ...
    for(int k=1; k<=t; k++)
    {
        ...
        sort(p,p+n);
        int counT = 2 ;
        /// Impossible if all points are Collinear
        for(int i=2; i<n; i++)
        {
        ...
        /// if not then possible
        vector<int>ans;
        vector<point>v = ConvexHull(n,p);
        map<int,int>vis;
        for(auto x : v)
        {
            int t = x.pos;
