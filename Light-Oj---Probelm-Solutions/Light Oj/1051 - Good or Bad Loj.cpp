...
int solve(int i,int con,int vow)
{
    counT++;
    if(i==s.sz)
    {
    ...
    int &ret = dp[i][con][vow];
    int &st = state[i][con][vow];
    ...
    {
        if(con<5)
            temp = max(temp,solve(i+1,0,vow+1));
        else
            temp = max(temp,solve(i+1,con,vow+1));

        if(vow<3)
            temp = max(temp,solve(i+1,con+1,0));
        else
            temp = max(temp,solve(i+1,con+1,vow));
    }
    else if(test(s[i]))
    {
         if(con<5)
