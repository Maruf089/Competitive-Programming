...
void solve()
{
    ...
    for(int mask = 1 ; mask < (1<<n) ; mask++)
    {
        f0(i,n)
        {
            ///Test whether a person is dead (dead is 0) If it is dead, continue
            if(checkBit(mask,i)==0)
                continue;
            ///Generate the state before killing this person, is to put the corresponding position from 1 becomes 0
            int now = mask ^ (1<<i);
            ///kill with the original gun
            dp[mask] = min(dp[mask] , dp[now] + health[i]);
            ///Use all the guns you have to kill to see if you can reduce the number of shots
            f0(j,n)
            {
                if(checkBit(now,j)==1)
                {
                    int temp = s[j][i] - '0';
                    if(temp>0)
                    dp[mask] = min(dp[mask] , dp[now] + (health[i]+temp-1)/temp);
                }
            }
        }
    }
