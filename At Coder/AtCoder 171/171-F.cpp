...
ll n,m,a,b,t,i,r,j,d,cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Max,Min,num;

ll fac[mx+9],inv[mx+9];
void fact()
{
    fac[0] = 1;
    inv[0] = modInverse(fac[0]);
    for(i=1;i<=mx;i++)
        {
...
{
  if (n < r)return 0;
  return modMul(fac[n], modMul(inv[n - r], inv[r]));
}
...
int main()
{
  ...
  for(i=n;i<=n+k;i++)
  {
      counT = modMul(modPow(26,d),modPow(25,k-d));
      counT = modMul(counT,NCR(i-1,r-1));
