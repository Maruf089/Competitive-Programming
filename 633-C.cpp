...
int main()
{
    ...
    for(int i=1; i<=n; i++)
    {
        ...
        for(int j=i; j>=1; j--)
        {
            int x = tolower(cipher[j]) -'a';
            if(tree[lvl][x]==0) break;
            lvl=tree[lvl][x];
            if(leaf_node[lvl] && dp[j-1]!=-1)
            {
                dp[i] = leaf_node[lvl];
                break;
            }
